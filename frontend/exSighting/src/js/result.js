...
document.addEventListener('DOMContentLoaded', () => {
    // 1. Dữ liệu giả lập (để test UI mà không cần backend chạy)
    const mockResults = [
    ...
    if (grid) {
        ...
        grid.innerHTML = mockResults.map(item => {
            // Chọn ảnh ngẫu nhiên theo theme
            let imgSrc = "https://images.unsplash.com/photo-1528127269322-539801943592?q=80&w=2070";
            if(item.themes.includes('mountain')) imgSrc = "https://images.unsplash.com/photo-1599229062397-6c8418047918?q=80&w=2070";
            if(item.themes.includes('city')) imgSrc = "https://images.unsplash.com/photo-1557750255-c76072a7aad1?q=80&w=2070";

            return `
            <div class="result-card" onclick="window.location.href='detail.html?id=${item.id}'" style="cursor: pointer;">
